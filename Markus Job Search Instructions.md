...
- **Job Sites → Run Search** is not automated the same way, and by design stays that way for now: there is no API or scraping integration for LinkedIn, Arbetsförmedlingen, Maquire, Novare or the other sources, and LinkedIn specifically cannot be automated at all — no public API, and scraping it violates their Terms of Service and gets blocked almost immediately. Clicking it opens each **Active** source's search results in a new browser tab, pre-filled with the combined terms: a real query string for LinkedIn and Arbetsförmedlingen (detected by source name), or the source's saved **Search URL** with the terms appended otherwise. A source with neither is skipped, and the status line names exactly what was skipped. It updates that source's **Last Run** date on every source it opens. Because it returns no data to the app, nothing from a Job Sites run gets filed into Job Watchlist — Candidate reviews the opened tabs personally and adds anything worth tracking by hand.

Never describe a Job Sites run as having "found" or "fetched" anything — it opens tabs for Candidate to review personally. Only the Companies scan actually retrieves results and files them.

Real integration for the remaining Job Sites (Arbetsförmedlingen has a genuine public API; the others could use the same scraping approach as Companies) was considered and deliberately deferred — Candidate chose to keep it manual for now rather than build and deploy the Cloud Function change.
