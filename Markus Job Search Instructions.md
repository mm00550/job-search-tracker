...
- On every other card, the button reads **"🔗 Open"** and opens that source's saved **Search URL** in a new tab, with the combined terms appended as a `q=` parameter. If no Search URL is saved, it does nothing but say so in the status line rather than silently failing — add one via Edit first.
...
Real integration for the remaining sources (most would need the same scraping approach Companies already uses) is possible but wasn't built — Arbetsförmedlingen was the one genuinely free win because of its open API; the others would need a Cloud Function change and deployment, which Candidate has not asked for.
