...
- **Job Sites → Run Search** is a hybrid, and the two halves must not be conflated when reporting back to Candidate:
  - **Arbetsförmedlingen** (if Active) is real, automated search — called directly from the browser against their public JobSearch API (`jobsearch.api.jobtechdev.se`), which is unauthenticated and fully CORS-open, so no Cloud Function or deployment is involved. One request per combined term (capped at 15 — a reasonable-citizen limit against a free public API), results merged and deduplicated by posting id, then filtered by location client-side (the API's own location filters need taxonomy concept ids, not free-text place names, so a substring match on the returned municipality/region is used instead). Every match is filed directly into **Job Watchlist** the same way as a Companies match — status `Watching`, deduplicated by company + role. Updates its own **Last Run** date.
  - **Every other Active source** (LinkedIn if re-added, Maquire, Novare, The Hub, career pages, anything else) still has no API or scraping integration. LinkedIn specifically cannot be automated at all — no public API, and scraping it violates their Terms of Service and gets blocked almost immediately. These open in a new browser tab, pre-filled with the combined terms: a real query string for LinkedIn (detected by source name), or the source's saved **Search URL** with the terms appended otherwise. A source with neither is skipped, and the status line names exactly what was skipped. Tabs are opened synchronously before the Arbetsförmedlingen fetch runs, so popup blockers don't treat them as unrequested.

Never describe the non-Arbetsförmedlingen part of a Job Sites run as having "found" or "fetched" anything — those sources only get an opened tab for Candidate to review personally. Only Companies and Arbetsförmedlingen actually retrieve and file results.
