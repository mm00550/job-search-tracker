...
- **Companies → Run Search** is real, automated search: it calls the deployed `scanWatchlist` Cloud Function, which fetches each watched company's career page (fast paths for Greenhouse/Lever/SmartRecruiters, generic scraping otherwise) and filters postings against the combined terms. Every match is filed directly as a new **Job Watchlist** opportunity (status `Watching`) — deduplicated against existing records by company + role, so re-running doesn't create repeats. There is no separate review-and-add step; a match becomes a real, trackable opportunity immediately. This is deliberately different from the older **Scan Watchlist** feature on the Suggestions tab, which still populates `state.suggestions` for manual review — that one is untouched.
- **Job Sites → Run Search** only searches **Arbetsförmedlingen**, and only if it's set Active — nothing else happens for other sources, by Candidate's explicit instruction. It's called directly from the browser against their public JobSearch API (`jobsearch.api.jobtechdev.se`), which is unauthenticated and fully CORS-open, so no Cloud Function or deployment is involved. One request per combined term (capped at 15 — a reasonable-citizen limit against a free public API), results merged and deduplicated by posting id, then filtered by location client-side (the API's own location filters need taxonomy concept ids, not free-text place names, so a substring match on the returned municipality/region is used instead). Every match is filed directly into **Job Watchlist** the same way as a Companies match — status `Watching`, deduplicated by company + role. Updates Arbetsförmedlingen's own **Last Run** date.

No other Job Site (LinkedIn if re-added, Maquire, Novare, The Hub, career pages) has any integration, automated or otherwise. The column-level Run Search button reflects that: it only ever touches Arbetsförmedlingen, and only when it's Active. Never describe another source as searched, opened, or checked by the column button — only Companies and Arbetsförmedlingen actually retrieve and file results there.

### Per-source Search/Open button

Every card in the Job Sites list also has its own button — a different, deliberate case from the column-level sweep above, and not contradicting the earlier instruction to remove automatic tab-opening (that was about an unattended sweep across every Active source; a single click on one named card is Candidate explicitly choosing that source, every time):

- On the **Arbetsförmedlingen** card, the button reads **"▶ Search"** and runs the exact same real API search as the column button, scoped to just that card. Works regardless of its Status (Active/Paused/Manual) — a direct click always means "search this one now."
- On every other card, the button reads **"🔗 Open"** and opens that source's saved **Search URL** in a new tab. If no Search URL is saved, it does nothing but say so in the status line rather than silently failing — add one via Edit first.

Both update that source's own **Last Run** date. Keep the button label meaningful if this is extended further — "Search" only where results actually come back into the app, "Open" everywhere a human is doing the searching themselves.

**Search URL is a template, not a plain link.** A URL containing `{q}` (and `{location}` where the site takes one) has those placeholders substituted with the combined search terms before opening — `resolveSearchUrl()` does this, shared by the card's own "🔗 Search page" link and the per-source Open button, so both are always consistent. A URL with no placeholder falls back to a generic `?q=` append, which happens to work for some sites (Indeed) and not others.

**Only LinkedIn and Indeed have confirmed templates**, verified directly against the live sites: `https://www.linkedin.com/jobs/search/?keywords={q}&location={location}` and `https://se.indeed.com/jobs?q={q}&l={location}`. Every other starter source either has no verified pattern (its Notes field says so explicitly — "Unverified, do one search on the site itself, copy the resulting URL...") or is a guessed `?q=`/`?text=` pattern also flagged as unconfirmed in Notes. Do not silently assume a guessed or unverified pattern actually filters results — check the Notes field before claiming a source is "searched." When Candidate confirms a working pattern by testing it themselves, update that source's Search URL with the real template and clear the "unverified" note — this is how the list gets more sources genuinely working over time, since the assistant cannot reliably discover many of these patterns itself (server-rendered sites can sometimes be checked directly; sites that render results via client-side JavaScript can't be verified this way at all, since the raw HTML looks the same regardless of query).

Real API integration for any of these (most would need the same scraping approach Companies already uses) is possible but wasn't built — Arbetsförmedlingen was the one genuinely free win because of its open, CORS-enabled API; the others would need a Cloud Function change and deployment, which Candidate has not asked for.
