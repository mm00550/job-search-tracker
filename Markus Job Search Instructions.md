...
No other Job Site (LinkedIn if re-added, Maquire, Novare, The Hub, career pages) has any integration, automated or otherwise — Run Search used to open a pre-filled tab for these, but that was deliberately removed. If another Active source has no automated search, the status line names it and nothing further happens for it; Candidate visits it manually using its saved Search URL if one exists. Never describe those sources as searched, opened, or checked — only Companies and Arbetsförmedlingen actually retrieve and file results.
