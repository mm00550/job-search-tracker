...
**Where this actually lives:** the **Job Sites & Companies** tab has two live sections that feed the search directly — **Role Titles** (a pinned, editable chip list of the titles Candidate's experience matches best, seeded from the Career Database) and **Search Criteria** (a free-text field plus quick-add tags for type of role, type of company, location and office policy). Clicking a chip in either section drops it into the same free-text field. This is stored as `searchCriteria.roleTitles` and `searchCriteria.freeText`.

**Only role titles are ever sent as search terms.** The Type of role, Type of company and Office policy chips (Full-time, Consultancy, Interim, Tech, Financial services, Public services, Life science, MedTech, Pharma, Automotive, Hybrid, On-site, Remote) describe qualifiers, not job titles — searched as a standalone term against a full-text job index they're far too generic and pull in unrelated postings (a real test: "Full-time" alone returned a waiter job as a top hit; "Senior Project Manager" alone returned only relevant senior delivery/programme roles). `buildSearchTerms()` filters these out via a known-qualifier list before anything is queried — never search a qualifier value on its own if extending this further. The Location chip (Gothenburg) is the one exception: clicking it also sets `searchCriteria.location`, the field actually used for location filtering, rather than becoming a search term itself.
...
The Job Sites & Companies tab has a **Run Search** button in each column. Both draw on the same combined terms from Step 1 above, and both file every match directly into **Job Watchlist** (status `Watching`) — no separate review-and-add step:

- **Companies → Run Search** calls the deployed `scanWatchlist` Cloud Function, which fetches each watched company's career page (fast paths for Greenhouse/Lever/SmartRecruiters, generic scraping otherwise) and filters postings against the combined terms. Deduplicated against existing records by company + role. This is deliberately different from the older **Scan Watchlist** feature on the Suggestions tab, which still populates `state.suggestions` for manual review — that one is untouched.
- **Job Sites → Run Search** runs every **Active** source through one of three paths:
  - **Arbetsförmedlingen**, if present and Active, is searched directly from the browser against their public JobSearch API (`jobsearch.api.jobtechdev.se`) — unauthenticated, fully CORS-open, no Cloud Function involved. One request per combined term (capped at 15), results deduplicated by posting id, filtered by location client-side (their location filters need taxonomy concept ids, not free-text place names).
  - **LinkedIn**, if present and Active, is never searched automatically at all — scraping it violates their Terms of Service. It's excluded from both paths below; only its own card's "Open" link reaches it, and that's a manual tab, not a search.
  - **Every other Active source with a saved Search URL** is sent, as a batch, to the `scanJobSites` Cloud Function — the same scraping approach as Companies, applied to a general job board's search-results page instead of a single company's career page. Company name often can't be reliably read off a general board's HTML (unlike a company's own career page, where it's already known), so a match with no discoverable company is filed as `via <source name>` rather than a fabricated one — an honest placeholder Candidate can correct once the posting is opened. Deduplicated primarily by posting URL, since a placeholder company name isn't a reliable identity key across many postings from the same board. Active sources with **no** Search URL saved are skipped, named in the status line.

`scanJobSites` requires deployment (`firebase deploy --only functions`) the same way `scanWatchlist` did when it was first built — if the assistant has just added or changed this function, say so plainly and that Candidate needs to deploy it; don't assume it's already live. If the request fails, the status line says so rather than silently reporting zero results as if the search had simply found nothing.

### Per-source Search/Open button

Every card in the Job Sites list also has its own button — a different, deliberate case from the column-level sweep above (a single click on one named card is Candidate explicitly choosing that source, not an unattended pass over everything Active):

- **Arbetsförmedlingen** and **every other source with a saved Search URL** get **"▶ Search"** — the same real search as the column button (API call or `scanJobSites` scrape respectively), scoped to just that one card, filed into Job Watchlist. Works regardless of Status (Active/Paused/Manual) — a direct click always means "search this one now."
- **LinkedIn** alone gets **"🔗 Open"** — opens its Search URL in a new tab. Nothing else does this anymore; once a source has a Search URL, clicking its card searches it for real rather than just opening it.
- A source with no Search URL saved does nothing but say so in the status line — add one via Edit first.

Both update that source's own **Last Run** date.

**Search URL is a template, not a plain link.** A URL containing `{q}` (and `{location}` where the site takes one) has those placeholders substituted with the combined search terms — `resolveSearchUrl()` does this client-side before either opening a tab (LinkedIn) or sending the resolved URL to `scanJobSites` (everything else), so the card's own "🔗 Search page" link and both Search buttons are always consistent with each other. A URL with no placeholder falls back to a generic `?q=` append, which happens to work for some sites and not others.

**Only LinkedIn and Indeed have confirmed templates**, verified directly against the live sites: `https://www.linkedin.com/jobs/search/?keywords={q}&location={location}` and `https://se.indeed.com/jobs?q={q}&l={location}`. Every other starter source either has no verified pattern (its Notes field says so explicitly — "Unverified, do one search on the site itself, copy the resulting URL...") or is a guessed pattern also flagged as unconfirmed. A guessed or unverified template is still sent to `scanJobSites` when its source is searched — the scrape may just return few or no results if the pattern doesn't actually filter, which is different from the source being broken. Do not claim a source's results are complete or precise without checking whether its pattern was ever confirmed. When Candidate confirms a working pattern by testing it themselves, update that source's Search URL with the real template and clear the "unverified" note.

**Some sources will return nothing even once deployed, and that's expected, not a bug.** `scanJobSites` fetches raw HTML server-side — it cannot execute JavaScript. A site that renders its listings client-side (several of the smaller Swedish/Gothenburg boards do) will look the same whether or not the query actually matched anything, the same reason those patterns couldn't be verified from outside a browser in the first place. Sites confirmed to expose real server-rendered listings in testing: Indeed, the two Gothenburg lediga-jobb boards, Offentliga Jobb, Randstad, Manpower, Bravura, Västra Götalandsregionen, Göteborgs Stad (Varbi), Michael Page. Blocket, Nature Careers, Poolia and Academic Work returned nothing in that same testing — don't assume they're wired correctly just because they're in the list.
